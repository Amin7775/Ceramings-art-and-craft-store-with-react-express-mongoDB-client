...
const MyListCard = ({item}) => {
    ...
    return (
        ...
        <div className="card-body items-start text-center">
          ...
          <p className="text-lg "><span className="font-medium">Customization : </span> {customization.toUpperCase()}</p>
          <p className="text-lg "><span className="font-medium">Stock Status : </span> {stockStatus}</p>
          <p className="text-lg "><span className="font-medium">Price : </span> {price} taka</p>
          <div className="w-full">
              <Link to={`/allitems/${_id}`}>
            <button className="btn w-full bg-custom-color-2 text-white mt-2 text-lg hover:bg-black/[.8]">Update</button>
              </Link>
              <Link to={`/allitems/${_id}`}>
            <button className="btn w-full bg-red-400 text-white mt-2 text-lg hover:bg-red-400/[.9]">Delete</button>
              </Link>
